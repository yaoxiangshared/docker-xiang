...
docker run -d --name nginx -p 80:80  --network my-net  --mount type=bind,source=/data/project/docker-xiang/docker-xiang/nginx/site,target=/etc/nginx/conf.d/site --mount type=bind,source=/data/project,target=/var/www  --mount type=bind,source=/usr/local/log,target=/var/log --add-host=host.docker.internal:host-gateway nginx:v1 
